...
5. **Use the built-in skills that are installed**, where they fit: `claude-md-management:claude-md-improver` for `CLAUDE.md` and `AGENTS.md`, `skill-creator` for the descriptions. Before anything that starts agent sessions — an evaluation run, a description-triggering loop — say how many sessions it will start, and wait for a yes.
