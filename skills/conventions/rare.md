...
- Count the outer holder over all of them; count a region only some render over
  those.
- **A region only the reference renders** takes the branch in *A kind whose family has only the reference*, below, for that region
...
- **Write nothing in it as `<n> of <m>`** — [SKILL.md](SKILL.md), *Counting honestly*.
